#!/usr/bin/env python3
"""修复 server.py 和 _convert.py 的编码问题"""
import os
...
if __name__ == "__main__":
    ...
    r = _convert_to_pcm(b"\\x00" * 320)
    print(f"Test OK: {len(r)} samples")
'''

with open("app/_convert.py", "w", encoding="utf-8") as f:
    f.write(convert_code.strip())
...
class FasterWhisperASRServicer(asr_service_pb2_grpc.FasterWhisperASRServicer):
    ...
    def RecognizeFile(self, request, context):
        logger.info("RecognizeFile: %s bytes, lang=%s", len(request.audio_data), request.language)
        ...
        text, segments, confidence = self.engine.transcribe_file(
            audio_np, request.language or None, request.enable_vad
        )
        proto_segments = []
        for seg in segments:
            words = [asr_service_pb2.WordTimestamp(
        ...
        logger.info("RecognizeFile done: text='%s', duration=%.2fs", text, duration)
        return asr_service_pb2.RecognizeResult(
            text=text, language=request.language or "auto",
            duration=duration, confidence=confidence, segments=proto_segments,
        )
        ...
        try:
            for chunk in request_iterator:
                ...
                if processor is None:
                    sid = chunk.session_id or "default"
                    processor = ASRStreamProcessor(engine=self.engine, sample_rate=16000)
                    self.stream_processors[sid] = processor
                if chunk.is_end:
                    break
                if chunk.audio_data:
                    for r in processor.process_chunk(chunk.audio_data):
                        yield _build_proto(r)
            if processor:
                for r in processor.finish():
                    yield _build_proto(r)
                processor.reset()
...
def _build_proto(r):
    words = [asr_service_pb2.WordTimestamp(
        word=w.word, start_time=w.start_time, end_time=w.end_time, probability=w.probability
    ) for w in (r.word_timestamps or [])]
    return asr_service_pb2.AsrResult(
        text=r.text, is_final=r.is_final, confidence=r.confidence,
        start_time=r.start_time, end_time=r.end_time, language=r.language,
        word_timestamps=words,
    )


def serve(port=50051, model_name="large-v2", device="cuda",
    ...
    servicer.engine = WhisperEngine.get_instance()
    asr_service_pb2_grpc.add_FasterWhisperASRServicer_to_server(servicer, server)
    server.add_insecure_port("0.0.0.0:%d" % port)
    server.start()
    logger.info("Server started: port=%s model=%s device=%s", port, model_name, device)
...
if __name__ == "__main__":
    ...
    p.add_argument("--device", default=os.getenv("ASR_DEVICE", "cuda"))
    ...
    if device == "cuda":
        try:
            ...
            device = "cpu"
    serve(a.port, a.model, device, a.compute_type, a.download_root)
'''

with open("app/server.py", "w", encoding="utf-8") as f:
    f.write(server_code.strip())
print("server.py rewritten OK")

# Verify
import app.server
print("server.py import OK")
import app._convert
print("_convert.py import OK")
